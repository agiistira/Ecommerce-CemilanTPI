			...
			<div class=" main-content-area">
                @if (Cart::instance('cart')->count() > 0)
                    <div class="wrap-iten-in-cart">
                            ...
                            @foreach(Cart::instance('cart')->Content() as $item)
                                <li class="pr-cart-item">
                                    ...
                                    <div class="quantity">
                                        ...
                                        <p class="text-center"><a href="#" wire:click.prevent="switchToSaveToLater('{{ $item->rowId }}')">Save To Later</a></p>
                                    </div>
                                    <div class="price-field sub-total"><p class="price">Rp. {{$item->subtotal}}</p></div>
                                    <div class="delete">
                                        <a href="#" class="btn btn-delete" title="">
                                            <span>Delete from your cart</span>
                                            <i class="fa fa-times-circle" aria-hidden="true" wire:click.prevent="destroy('{{ $item->rowId }}')"></i>
                                        </a>
                                    </div>
                                </li>
                            @endforeach
                        ...
                        @else
                        <p class="alert alert-danger">No item in cart</p>
                        @endif
                    </div>
                    ...
                    <div class="summary">
                        <div class="order-summary">
                            ...
                            @if (Session::has('coupon'))
                                ...
                                <p class="summary-info"><span class="title">Subtotal With Discount </span><b class="index">Rp. {{ number_format($subtotalAfterDiscount,2) }}</b></p>
                                <p class="summary-info"><span class="title">Tax ({{ config('cart.tax') }}% )</span><b class="index">${{ number_format($taxAfterDiscount,2) }}</b></p>
                                <p class="summary-info"><span class="title">Total </span><b class="index">Rp. {{number_format($totalAfterDiscount,2)}}</b></p>
                            @else
                                {{-- <p class="summary-info"><span class="title">Tax</span><b class="index">Rp. {{ Cart::instance('cart')->tax() }}</b></p> --}}
                                {{-- <p class="summary-info"><span class="title">Shipping</span><b class="index">Free Shipping</b></p> --}}
                                <p class="summary-info"><span class="title">Total</span><b class="index">Rp. {{ Cart::instance('cart')->subtotal() }}</b></p>
                            @endif
                        </div>
                ...
                <div class="wrap-iten-in-cart">
							...
							<li class="pr-cart-item">
								...
								<div class="delete">
									<a href="#" class="btn btn-delete" title="">
										...
										<i class="fa fa-times-circle" aria-hidden="true" wire:click.prevent="deletedFromSaveToLater('{{ $item->rowId }}')"></i>
									</a>
								</div>
							</li>
						@endforeach
					...
					@else
					<p class="alert alert-danger">No item saved for later</p>
					@endif
				</div>
